        ...
        return `<a href="/prompt/${item.id}" class="block card rounded-lg p-3 shadow-sm group">
            ...
            <div class="flex items-center justify-between pt-2 mt-2.5" style="border-top: 1px solid var(--border)">
                <span onclick="goToProfile('${item.uploadedBy}', event)" class="author-chip flex items-center gap-0.5 cursor-pointer">
                    ${profilePicHtml}
                    <span class="text-xs font-semibold card-author" style="color:var(--text-secondary)">@${item.uploadedBy}</span>${item.isAdmin ? '<img src="/assets/verified.svg" style="width:11px;height:11px;flex-shrink:0;" alt="✓">' : ''}
                </span>
